...
/* ═══════════════════════════════════════════════════════════════════
   ...
   variables are assigned AGAIN, and then restoreScreenUI() paints the answered look. goTo() now
   runs that same snapshot → re-apply → paint sequence on EVERY navigation, which is what keeps an
   already-answered screen answered when the learner returns to it — both mid-session and after a
   reload, when only the restored variables know the answer. The sNNEnter() functions are still
