...
function goTo(n) {
  ...
  /* Keep an answered screen answered when the learner returns to it.
     sNNEnter() is an INITIALISER: it zeroes this screen's answer variables and wipes its
     DOM. Snapshot before it runs, re-apply after, then let the existing painter rebuild the
     answered look — the same three steps applyExecutionState() already does for the landing
     screen, now applied to every navigation. That is what makes a REVISITED screen keep its
     final state, and what rebuilds it after a reload, when the DOM is pristine markup and
     only the restored variables know the answer.
     s39/s40/s41 already early-return on their solved flag, so for those the re-apply is a
     no-op and only the painter matters; s37/s38 have no such guard and rely on both halves.
...
/* ═══════════════════════════════════════════════════════════════════
   ...
   paint sequence on EVERY navigation, which is what keeps an already-answered screen answered
   when the learner returns to it — for s37/s38 the re-apply does the work, and for the guarded
   s39/s40/s41 the painter does, since after a reload they leave pristine markup on screen.
